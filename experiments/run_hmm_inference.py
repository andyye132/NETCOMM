...
# variant -> (method_yaml, overrides_dict)
VARIANTS = {
    "no_filter":      ("no_hmm",         {}),
    "2state_hmm":     ("adaptive",       {"n_regimes": 2}),
    "4state_hmm":     ("adaptive",       {"n_regimes": 4}),
...
def main():
    ...
    for scn in SCENARIOS:
        for variant, (mth, ov) in VARIANTS.items():
            ...
            for trial in range(N_TRIALS):
                key = jax.random.PRNGKey(hash(("hmm", scn, variant, trial)) & 0xFFFFFFFF)
                t0 = time.perf_counter()
                out = run_episode(cfg, controller, flows, N_STEPS, key)
                wall = time.perf_counter() - t0
                rows.append(dict(
                    scenario=scn, variant=variant, trial=trial,
                    ...
                    regime_acc=float(out.get("regime_accuracy", float("nan"))),
                    wall=float(wall),
                ))
            write_parquet(rows, TABLE)
            print(f"[{scn}|{variant}] {N_TRIALS} trials done")
    write_parquet(rows, TABLE)
    print("DONE hmm_inference")
